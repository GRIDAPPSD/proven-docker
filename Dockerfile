...
RUN echo $TIMESTAMP > /dockerbuildversion.txt \
    ...
    && mkdir /build \
    && cd /build \
    && git -c advice.detachedHead=false clone https://github.com/pnnl/proven-message.git -b query-filter --single-branch \
    ...
    && cd /build \
    && git -c advice.detachedHead=false clone https://github.com/pnnl/proven-cluster.git -b query-filter --single-branch \
